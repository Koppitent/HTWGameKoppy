	...
	if help_total_items > 0 and help_current_index >= 0 and help_current_index < help_total_items {
		...
		if help_image != "" {
			...
			if help_image_index >= 0 {
				...
				var image_width = 320;  // 16:9 ratio, much bigger width
				var image_height = 180; // 16:9 ratio, much bigger height
				
				var sheet_x = (help_image_index mod images_per_row) * image_width;
				var sheet_y = floor(help_image_index / images_per_row) * image_height;
				
				// Zeichne das Bild aus dem Sprite Sheet
				draw_sprite_part(sp_help, 0, sheet_x, sheet_y, image_width, image_height, 
					center_x - image_width/2, picture_y - image_height/2);
			} else {
				// Fallback wenn Index ungültig
				sc_drawrectangle(center_x - 100, picture_y - 50, center_x + 100, picture_y + 50, global.color_black, global.color_character, 1, 0.3, 0.7, 0);
				sc_drawtext(center_x, picture_y, "[Invalid image index]", global.color_white, global.color_black, 1, 1, 0, -1);
			}
		} else {
			// Kein Bild definiert
			sc_drawrectangle(center_x - 100, picture_y - 50, center_x + 100, picture_y + 50, global.color_black, global.color_character, 1, 0.3, 0.7, 0);
			sc_drawtext(center_x, picture_y, "[No Image]", global.color_white, global.color_black, 1, 1, 0, -1);
		}
		
		// Beschreibung anzeigen (mit automatischen Zeilenumbrüchen)
		var text_y = center_y + 10;
		var current_description = help_active_items[help_current_index,0];
		
		// Text in Zeilen aufteilen (maximal 5 Zeilen)
		var max_chars_per_line = 50; // Maximale Zeichen pro Zeile
		var max_lines = 5;
		var lines = [];
		var words = [];
		var word_count = 0;
		...
		var temp_word = "";
		for (var c = 1; c <= string_length(current_description); c++) {
			var char = string_char_at(current_description, c);
			if char == " " {
				if temp_word != "" {
					words[word_count] = temp_word;
					word_count++;
					temp_word = "";
				}
			} else {
		...
		var current_line = "";
		var line_count = 0;
		for (var w = 0; w < word_count && line_count < max_lines; w++) {
		...
		if current_line != "" && line_count < max_lines {
			lines[line_count] = current_line;
			line_count++;
		}
		...
		// Zeilen anzeigen
		var line_height = 12;
		var total_text_height = line_count * line_height;
		var start_y = text_y - (total_text_height / 2);
		
		for (var l = 0; l < line_count; l++) {
			sc_drawtext(center_x, start_y + (l * line_height), lines[l], global.color_white, global.color_black, 1, 1, 0, -1);
		}
		
		// Seitenangabe anzeigen
		var page_info = "Page " + string(help_current_index + 1) + " of " + string(help_total_items);
		sc_drawtext(center_x, center_y + 70, page_info, global.color_character_light, global.color_black, 1, 1, 0, -1);
		
		// Navigation anzeigen
		var nav_text = "[A/D] Navigate  [Esc] Close";
		sc_drawtext(center_x, center_y + 90, nav_text, global.color_character_light, global.color_black, 1, 1, 0, -1);
		
		// Pfeile für Navigation
		if help_current_index > 0 {
			sc_drawtext(center_x - 200, center_y, "< BACK", global.color_white, global.color_black, 1, 1, 0, -1);
		}
		if help_current_index < help_total_items - 1 {
			sc_drawtext(center_x + 200, center_y, "NEXT >", global.color_white, global.color_black, 1, 1, 0, -1);
		}
	} else {
		// Fallback wenn keine Hilfedaten vorhanden
		var center_x = camera_get_view_x(view_camera[0]) + cam_w/2;
		var center_y = camera_get_view_y(view_camera[0]) + cam_h/2;
		sc_drawtext(center_x, center_y, "No help data available.", global.color_white, global.color_black, 1, 1, 0, -1);
	}
}
//
