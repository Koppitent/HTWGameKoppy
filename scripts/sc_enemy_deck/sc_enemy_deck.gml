...
//————————————————————————————————————————————————————————————————————————————————————————————————————
	...
	if ob_main.stats_league_won=0 {
		for (var i=0; i<enemycard_maindeck_total; i++;) {
			if i<1 { sc_enemy_deck_addcard(i,549,4,ref_glyph_adaptability,ref_glyph_harvest,-1,1,0); } //LILLIGANT, LV 4 (>MAX) (IN 1)
		...
		enemycard_maindeck_total=40;
		for (var i=0; i<enemycard_maindeck_total; i++;) {
			if i<1 { sc_enemy_deck_addcard(i,549,10,ref_glyph_adaptability,ref_glyph_harvest,-1,innate_max,0); } //LILLIGANT, LV 10 (IN MAX)
...
//————————————————————————————————————————————————————————————————————————————————————————————————————
	...
	if ob_main.stats_league_won=0 {
		for (var i=0; i<enemycard_maindeck_total; i++;) {
			...
			else if i<3 { sc_enemy_deck_addcard(i,310,7,ref_glyph_magnetism_m,-1,-1,1,0); } //MANECTRIC, LV 7 (>MAX) (IN 1)
			else if i<4 { sc_enemy_deck_addcard(i,181,6,ref_glyph_magnetism_p,-1,-1,3,0); } //AMPHAROS, LV 6 (IN 3)
		...
		for (var i=0; i<enemycard_maindeck_total; i++;) {
			...
			else if i<3 { sc_enemy_deck_addcard(i,310,10,ref_glyph_magnetism_m,-1,-1,innate_max,0); } //MANECTRIC, LV 10 (IN MAX)
			else if i<4 { sc_enemy_deck_addcard(i,181,10,ref_glyph_magnetism_p,-1,-1,innate_max,0); } //AMPHAROS, LV 10 (IN MAX)
			else if i<10 { sc_enemy_deck_addcard(i,-1,10,-2,-2,-2,innate_max,-1); } //(6) LV 10 (IN MAX)
			else if i<30 { sc_enemy_deck_addcard(i,-1,10,-2,-2,-2,-1,-1); } //(20) LV 10
			else if i<40 { sc_enemy_deck_addcard(i,-1,9,-2,-2,-2,-1,-1); } //(10) LV 9
		}
...
//————————————————————————————————————————————————————————————————————————————————————————————————————
	...
	if ob_main.stats_league_won=0 {
		for (var i=0; i<enemycard_maindeck_total; i++;) {
			if i<1 { sc_enemy_deck_addcard(i,620,8,ref_glyph_counter,ref_glyph_courage,-1,2,0); } //MIENSHAO, LV 8 (>MAX) (IN 2)
			else if i<2 { sc_enemy_deck_addcard(i,538,8,-1,-1,-1,1,0); } //THROH, LV 8 (>MAX) (IN 1)
			else if i<3 { sc_enemy_deck_addcard(i,539,8,-1,-1,-1,1,0); } //SAWK, LV 8 (>MAX) (IN 1)
			else if i<4 { sc_enemy_deck_addcard(i,560,7,ref_glyph_intimidate,ref_glyph_memento,-1,3,0); } //SCRAFTY, LV 7 (IN 3)
		...
		enemycard_maindeck_total=40;
		for (var i=0; i<enemycard_maindeck_total; i++;) {
			if i<1 { sc_enemy_deck_addcard(i,620,10,ref_glyph_counter,ref_glyph_courage,-1,innate_max,0); } //MIENSHAO, LV 10 (IN MAX)
			else if i<2 { sc_enemy_deck_addcard(i,538,10,-1,-1,-1,innate_max,0); } //THROH, LV 10 (IN MAX)
			else if i<3 { sc_enemy_deck_addcard(i,539,10,-1,-1,-1,innate_max,0); } //SAWK, LV 10 (IN MAX)
			else if i<4 { sc_enemy_deck_addcard(i,560,10,ref_glyph_intimidate,ref_glyph_memento,-1,innate_max,0); } //SCRAFTY, LV 10 (IN MAX)
			else if i<10 { sc_enemy_deck_addcard(i,-1,10,-2,-2,-2,innate_max,-1); } //(6) LV 10 (IN MAX)
			else if i<30 { sc_enemy_deck_addcard(i,-1,10,-2,-2,-2,-1,-1); } //(20) LV 10
			else if i<40 { sc_enemy_deck_addcard(i,-1,9,-2,-2,-2,-1,-1); } //(10) LV 9
		}
...
//————————————————————————————————————————————————————————————————————————————————————————————————————
	...
	if ob_main.stats_league_won=0 {
		for (var i=0; i<enemycard_maindeck_total; i++;) {
			...
			else if i<4 { sc_enemy_deck_addcard(i,357,10,ref_glyph_forage,-1,-1,1,0); } //TROPIUS, LV 10 (>MAX) (IN 1)
		...
		for (var i=0; i<enemycard_maindeck_total; i++;) {
			...
			else if i<4 { sc_enemy_deck_addcard(i,357,10,ref_glyph_forage,-1,-1,innate_max,0); } //TROPIUS, LV 10 (IN MAX)
...
//————————————————————————————————————————————————————————————————————————————————————————————————————
	...
	if ob_main.stats_league_won=0 {
		for (var i=0; i<enemycard_maindeck_total; i++;) {
			...
			else if i<4 { sc_enemy_deck_addcard(i,571,10,ref_glyph_inertia,ref_glyph_curse,-1,innate_max,0); } //ZOROARK, LV 10 (IN MAX)
			else if i<5 { sc_enemy_deck_addcard(i,579,10,ref_glyph_guru,ref_glyph_motivation,-1,innate_max,0); } //REUNICLUS, LV 10 (IN MAX)
...
//————————————————————————————————————————————————————————————————————————————————————————————————————
	...
	if ob_main.stats_league_won=0 {
		for (var i=0; i<enemycard_maindeck_total; i++;) {
			...
			else if i<5 { sc_enemy_deck_addcard(i,534,10,-1,-1,-1,innate_max,0); } //CONKELDURR, LV 10 (IN MAX)
			else if i<24 { sc_enemy_deck_addcard(i,-1,10,-2,-2,-2,-1,-1); } //(19) LV 10
			else if i<36 { sc_enemy_deck_addcard(i,-1,9,-2,-2,-2,-1,-1); } //(12) LV 9
			else if i<40 { sc_enemy_deck_addcard(i,-1,8,-2,-2,-2,-1,-1); } //(4) LV 8
		}
...
//————————————————————————————————————————————————————————————————————————————————————————————————————
	...
	if ob_main.stats_league_won=0 {
		for (var i=0; i<enemycard_maindeck_total; i++;) {
			...
			else if i<4 { sc_enemy_deck_addcard(i,635,10,ref_glyph_might,ref_glyph_courage,-1,innate_max,0); } //HYDREIGON, LV 10 (IN MAX)
			else if i<5 { sc_enemy_deck_addcard(i,612,10,-1,-1,-1,innate_max,0); } //HAXORUS, LV 10 (IN MAX)
			else if i<24 { sc_enemy_deck_addcard(i,-1,10,-2,-2,-2,-1,-1); } //(19) LV 10
			else if i<36 { sc_enemy_deck_addcard(i,-1,9,-2,-2,-2,-1,-1); } //(12) LV 9
			else if i<40 { sc_enemy_deck_addcard(i,-1,8,-2,-2,-2,-1,-1); } //(4) LV 8
		}
